/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { ColumnCreateNestedManyWithoutDataTablesInput } from "./ColumnCreateNestedManyWithoutDataTablesInput";
import { Type } from "class-transformer";
import { AnalysisResultCreateNestedManyWithoutDataTablesInput } from "./AnalysisResultCreateNestedManyWithoutDataTablesInput";
import { RecipeExecutionCreateNestedManyWithoutDataTablesInput } from "./RecipeExecutionCreateNestedManyWithoutDataTablesInput";

@InputType()
class DataTableCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => ColumnCreateNestedManyWithoutDataTablesInput,
  })
  @ValidateNested()
  @Type(() => ColumnCreateNestedManyWithoutDataTablesInput)
  @IsOptional()
  @Field(() => ColumnCreateNestedManyWithoutDataTablesInput, {
    nullable: true,
  })
  columns?: ColumnCreateNestedManyWithoutDataTablesInput;

  @ApiProperty({
    required: false,
    type: () => AnalysisResultCreateNestedManyWithoutDataTablesInput,
  })
  @ValidateNested()
  @Type(() => AnalysisResultCreateNestedManyWithoutDataTablesInput)
  @IsOptional()
  @Field(() => AnalysisResultCreateNestedManyWithoutDataTablesInput, {
    nullable: true,
  })
  analysisResults?: AnalysisResultCreateNestedManyWithoutDataTablesInput;

  @ApiProperty({
    required: false,
    type: () => RecipeExecutionCreateNestedManyWithoutDataTablesInput,
  })
  @ValidateNested()
  @Type(() => RecipeExecutionCreateNestedManyWithoutDataTablesInput)
  @IsOptional()
  @Field(() => RecipeExecutionCreateNestedManyWithoutDataTablesInput, {
    nullable: true,
  })
  recipeExecutions?: RecipeExecutionCreateNestedManyWithoutDataTablesInput;
}

export { DataTableCreateInput as DataTableCreateInput };
